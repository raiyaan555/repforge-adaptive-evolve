...
export function WorkoutLog() {
  ...
  // ✅ UPDATED: Enhanced RPE-based progression with underperformance handling
  ...
  // ✅ UPDATED: Calculate expected reps for new sets with current week handling
  const calculateNewSetExpectedReps = (exercise: WorkoutLog, newSetRPE: number): number => {
    try {
      // ✅ NEW: For current week, new sets should have planned reps (but stay empty in UI)
      if (currentWeek === 1) {
  ...
  // ✅ ENHANCED: Updated initializeWorkoutLogs with underperformance logic and isNewSet tracking
  const initializeWorkoutLogs = async (workoutData: any, actualWeek: number, actualDay: number) => {
    try {
      ...
      // ✅ ENHANCED: Apply progression logic with underperformance handling
      const updatedLogs = [];
      for (const log of baseLogs) {
        try {
          ...
          if (recentExercise) {
            ...
            // ✅ ENHANCED: RPE-based rep progression with underperformance logic
            if (!isDeloadWeek) {
      ...
      // ✅ UPDATED: Apply muscle group-based set adjustments with enhanced new set tracking
      const isDeloadWeek = actualWeek === workoutData.duration_weeks;
  ...
  const addSet = (exerciseIndex: number) => {
    setWorkoutLogs(prevLogs => {
      try {
        ...
        // ✅ UPDATED: New sets added during current week stay empty (0 expected reps shown in UI)
        ...
        if (currentWeek === 1) {
          ...
          // Week 2+: Calculate based on existing sets (but will stay empty in UI for current week)
