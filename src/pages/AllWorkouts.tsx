...
export function AllWorkouts() {
  ...
  const handleStartPlan = async (workout: DefaultWorkout | CustomWorkout, workoutType: 'default' | 'custom') => {
    ...
    try {
      ...
      toast({
        title: "Workout plan started! 🎉",
        description: `${workout.name} is now your active mesocycle.`,
      });
      ...
      toast({
        ...
        description: "There was an error starting your workout plan. Please try again.",
        variant: "destructive"
      });
    } finally {
  ...
  const WorkoutCard = ({ workout, isCustom = false }: { workout: DefaultWorkout | CustomWorkout; isCustom?: boolean }) => {
    ...
    const isStarting = startingWorkout === workout.id;
                  ...
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      ...
                      >
                        ...
                        {isStarting ? 'Starting...' : 'Start Plan'}
                      </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Start {workout.name}?</AlertDialogTitle>
                        <AlertDialogDescription>
                          This will make {workout.name} your active workout plan. You can track your progress and log workouts for this plan.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction 
                          onClick={() => handleStartPlan(workout, isCustom ? 'custom' : 'default')}
                        >
                          Start Plan
                        </AlertDialogAction>
                      </AlertDialogFooter>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold mb-4">All Workouts</h1>
